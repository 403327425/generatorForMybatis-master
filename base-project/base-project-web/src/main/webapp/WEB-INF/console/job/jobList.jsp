...
<div class="layui-fluid">
	<div class="search-wrap layui-form" >
		<div class="layui-input-inline">
		  	<input class="layui-input" name="jobName" id="jobName" placeholder="职位名称" autocomplete="off">
		</div>			
		...
		<div class="container">
			<table id="job" lay-filter="job" class="layui-hide"></table>
		</div>
	</div>
	...
	<script type="text/javascript">
	    layui.config({
			...
			var table = layui.table,
				...
				table.render({
					elem: '#job',
					url: '${root}/job/getData', //数据接口
					page: true, //开启分页
					limit:20, //初始显示20条数据	
					height: 'full-88',
					id:'job',
					cols: [[ //表头
						{field: 'jobName',minWidth:250,align:'center', title: '职位名称'},
						{field: 'superiorName',minWidth:250,align:'center', title: '上级职位'},
						{field: 'creatorName',minWidth:150,align:'center', title: '创建人'},	
						{field: 'createTime',minWidth:250,align:'center', title: '创建时间',templet : '<div>{{ formatTime(d.createTime,"yyyy-MM-dd hh:mm:ss")}}</div>'},
						{fixed: 'right', title:"操作",minWidth:150, align:'left', toolbar: '#operate'},
					]]
				});
				...
				table.on('tool(job)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if (layEvent === 'edit') { //编辑
						layer.open({
						  	type: 2,
				          	title: '编辑职位',
				          	content: '${root}/job/toUpdate?jobId='+obj.data.jobId,
				          	maxmin: true,
	    ...
	    function dataAction(data){
	    	var url = "";
	    	if(data.jobId == null || data.jobId == "" || data.jobId == undefined){
	    		url = '${root}/job/insert';
	    	}else{
	    		url = '${root}/job/update';
	    	}
